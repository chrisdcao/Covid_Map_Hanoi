...
###########################################################
##                                                       ##
##  FILE ĐỂ XÂY DỰNG VECTOR DỰA TRÊN CÁC FEATURE CỦA TỪ  ##
##                                                       ##
###########################################################

""" Feature Representation của các từ
        Args:
            feature: số đặc tính chúng ta muốn biểu diễn độ tương quan dựa trên (trong paper có giải thích)
        Input:
            vocab_síze: độ to vốn từ vựng của mô hình đã đc nhận
            embedding_dim: số feature đầu ra mà chúng ta muốn vector biểu diễn theo
        Output:
            feat_embds: vector đã đc biểu diễn theo feature (thay vì one-hot-encode)
"""
