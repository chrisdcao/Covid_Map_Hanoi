...
""" Xây dựng class con lấy BertPreTrainedModel làm class cha
        Args:
            config: cài đặt, fetch mặc định từ mô hình BERT 
            feature: số đặc tính để biểu diễn vector
            use_lstm: sử dụng LSTM hay không, mặc định là không (nhưng sẽ điền vào True ở phần sau, khi chạy mô hình)
            device: cpu hoặc gpu 
        Input:
            input_ids: id của đầu vào raw
            attention_masks: mặt nạ cho các điểm chú ý trong câu
            token_mask: mặt nạ cho các phân đoạn của câu
            segment_ids: id của các phân đoạn câu
            label_masks: mặt nạ cho nhãn dán câu
            feats: số đặc tính để biểu diễn vector
        Output:
            active_logits: Các vector đã được chuẩn hóa đang hoạt động 
"""
...
""" Hàm xây mô hình
        Input: 
            model_name_or_path: đường dẫn tới mô hình (code)
            num_labels: số nhãn dán (= số nhãn trong conLL format 2003)
            one_hot_embed: sử dụng 0 và 1 để biểu diễn từ, mặc định true
            use_lstm: có dùng mạng lstm hay không
            device: cpu hoặc gpu nếu có cuda
        Output:
            config: cài đặt mô hình
            model: mô hình
            feature: số đặc tính(hay số chiều) biểu diễn vector
"""
...
""" Xây model từ model đã đc train
"""
